...
`invoice-line-items`, `payments`. Field shapes are exactly the sync `fields` payloads
documented below, plus the DRF-standard `id`, `created_at`, `updated_at`, `deleted_at`.

## Sync

Six syncable models in this slice, referenced by these `model` keys:
`lead`, `customer`, `quote`, `quote_line_item`, `job`, `invoice`, `invoice_line_item`,
`payment` (8 keys). All are scoped to the caller's business server-side; a client never sends
`business`.
...
line item → payment) so a line item ahead of its not-yet-applied parent in the list still
resolves correctly. The one case this doesn't cover: converting a lead to a customer where
...
VAT: flat 15%, applied to `subtotal - discount_amount` when `is_vat_applicable` is true.
